...
var setPassportStrategy = function(){
	...
	AccessToken.findAccessTokenByValue(accessToken).then(function(token){
		...
		throw AppError.throwAppError(403);
            }
	    return token.getAuth().then(function(authInfo){
		if(!authInfo){
		    throw AppError.throwAppError(404);
		}
		if(authInfo.isBan){
		    throw AppError.throwAppError(401);
		}
	    ...
	    log.error("#bearerAuth", {aceessToken : accessToken}, {err : err.toString()}, {stack : err.stack});
	    done(null, false);
	});
    }));
    ...
    passport.use(new ResourceOwnerPasswordStrategy({
        passReqToCallback: true
    }, function(req, client_id, client_secret, accessToken, authType, done) {
	...
	case define.kind.type.refreshToken.name:
        ...
        Auth.authenticate(accessToken, authType).then(function(authInfo) {
            return Auth.verifyClient(authInfo, client_id, client_secret).then(function (client) {
                done(null, authInfo, client);
            });
        }).catch(function(err){
