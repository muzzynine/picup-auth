...
var setPassportStrategy = function(){
    ...
    passport.use(new BearerStrategy({
        ...
        function(accessToken, done){
            AccessToken.findAccessTokenByValue(accessToken, function(err, token){
                if(err){
                    //액세스 토큰이 존재하지 않을때 401리턴
                    return done(err);
                }
                User.findUserById(token.user_id, function(err, user){
                    if(err){
                        //매치되는 유저가 없을때 404리턴
                        return done(err);
                    }
    ...
    passport.use(new ResourceOwnerPasswordStrategy({
        ...
        if (!grant_type) {
            return done(AppError.throwAppError(400));
        }
        switch (grant_type) {
